...
def frontpage():
    if 'loggedin' in session:
        ...
        try:
            with mysql.cursor() as cursor:
                cursor.execute("SELECT * FROM leaderboard WHERE student_id = %s", (student_id,))
                student_in_leaderboard = cursor.fetchone()
                
                if not student_in_leaderboard:
                    initial_exp_points = 0
                    cursor.execute("INSERT INTO leaderboard (student_id, exp_points, progress_percentage) VALUES (%s, %s, %s)", (student_id, initial_exp_points, 0,))
                    mysql.commit()
...
@app.route('/topic/<course_name>')
def topic(course_name):
    if 'loggedin' in session:
        student_id = session['student_id']

        course_data = courses.get(course_name)
        if course_data:
            return render_template('topic.html', course=course_data)
        else:
...
def check_assignment():
    ...
    try:
        with mysql.cursor() as cursor:
            cursor.execute("SELECT * FROM completed_assgn WHERE student_id = %s AND assignment_id = %s AND topic_id = %s", (student_id, assignment_number, topic_id))
    ...
    try:
        with mysql.cursor() as cursor:
            ...
            if assignment_row:
                ...
                cursor.execute("INSERT INTO completed_assgn (student_id, assignment_id, topic_id) VALUES (%s, %s, %s)", (student_id, assignment_id, topic_id))
                mysql.commit()

                cursor.execute("UPDATE leaderboard SET exp_points = exp_points + %s WHERE student_id = %s", (exp_points_awarded, student_id))
                ...
                cursor.execute("SELECT COUNT(*) FROM completed_assgn")
    ...
    if 'loggedin' in session:
        ...
        try:
            with mysql.cursor() as cursor:
                ...
                if exp_points == None:
                    return render_template('dashboard.html', username=session.get("username"), exp_points=0)
...
def dashboard_data():
    try:
        with mysql.cursor() as cursor:
            cursor.execute("""
                SELECT s.student_name, l.exp_points
                FROM leaderboard l
                JOIN student s ON l.student_id = s.student_id
                ORDER BY l.exp_points DESC
            """)
            leaderboard_data = cursor.fetchall()
            leaderboard_data = [{"student_name": row[0], "exp_points": row[1]} for row in leaderboard_data]
...
# adding route to assignment-page
@app.route('/assignment/<assignment_type>')
def assignment_page(assignment_type):
    
    if 'loggedin' in session:
        if assignment_type == 'MCQ':
            return render_template('assignment_mcq.html')
        else:
