...
function AssignmentsMCQ() {
  const navigate = useNavigate();
  const location = useLocation();
  const initialTopicName: string = location.state?.topic_name || "";
  const [marks, setMarks] = useState<number>(0);
  var [temp_qs, setQuestions] = useState<{ question: string; options: string[]; answer: string; selectedAns: string; }[]>(mcq_questions[initialTopicName as keyof typeof mcq_questions]);

  useEffect(() => {
    const selectedQuestions = mcq_questions[initialTopicName as keyof typeof mcq_questions];
  ...
  const completeAssignment = (event: React.FormEvent<HTMLFormElement>, marks: number) => {
    event.preventDefault();
    APIService.PostData(
      { topic_name: location?.state.topic_name, marks: marks, assignment_name: "MCQ" },
      "/complete-assignment"
    ).then((res: any) => {
      console.log(res);
    });
